...
public:
  ...
  /*implicit*/ Array2X(std::size_t nx, std::size_t ny)
      : _size{nx, ny}, _data(nx * ny) {}
  /*implicit*/ Array2X(std::size_t nx, std::size_t ny, const T &value)
      : _size{nx, ny}, _data(nx * ny, value) {}
  ...
  template <class IntType> const T &operator()(IntType i, IntType j) const {
    CORRADE_INTERNAL_ASSERT(i >= 0 && j >= 0 && std::size_t(i) < _size[0] &&
                            std::size_t(j) < _size[1]);
  ...
  const T &operator()(const Math::Vector2<IntType> &coord) const {
  ...
  template <class IntType> void resize(IntType nx, IntType ny, const T &value) {
    std::size_t _tmp[2] = {static_cast<std::size_t>(nx), static_cast<std::size_t>(ny)};

    std::copy(std::begin(_tmp), std::end(_tmp), std::begin(_size));
